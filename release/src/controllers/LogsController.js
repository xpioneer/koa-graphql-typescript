"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var typeorm_1 = require("typeorm");
var Moment = require("moment");
var api_1 = require("../entities/mongo/api");
var errors_1 = require("../entities/mongo/errors");
var tools_1 = require("../utils/tools");
var LogsController = /** @class */ (function () {
    function LogsController() {
    }
    LogsController.getById = function (id) {
        if (id === void 0) { id = ''; }
        return __awaiter(this, void 0, void 0, function () {
            var api;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, typeorm_1.getMongoRepository(api_1.API, 'mongo').findOne({ id: id })];
                    case 1:
                        api = _a.sent();
                        return [2 /*return*/, api];
                }
            });
        });
    };
    LogsController.apiPages = function (ctx) {
        return __awaiter(this, void 0, void 0, function () {
            var params, query, options, pages;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        params = ctx.getParams;
                        query = ctx.query;
                        options = {
                            skip: params.offset,
                            take: params.limit,
                            order: {
                                createdAt: 'DESC'
                            },
                            where: {}
                        };
                        if (query.path) {
                            options.where['path'] = query.path;
                        }
                        if (query.url) {
                            options.where['url'] = query.url;
                        }
                        return [4 /*yield*/, typeorm_1.getMongoRepository('API', 'mongo').findAndCount(options)];
                    case 1:
                        pages = _a.sent();
                        ctx.Pages({ page: pages });
                        return [2 /*return*/];
                }
            });
        });
    };
    LogsController.errorsPages = function (ctx) {
        return __awaiter(this, void 0, void 0, function () {
            var params, query, options, pages;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        params = ctx.getParams;
                        query = ctx.query;
                        options = {
                            skip: params.offset,
                            take: params.limit,
                            order: {
                                createdAt: 'DESC'
                            },
                            where: {}
                        };
                        if (query.path) {
                            options.where['path'] = query.path;
                        }
                        if (query.url) {
                            options.where['url'] = query.url;
                        }
                        return [4 /*yield*/, typeorm_1.getMongoRepository('Errors', 'mongo').findAndCount(options)];
                    case 1:
                        pages = _a.sent();
                        ctx.Pages({ page: pages });
                        return [2 /*return*/];
                }
            });
        });
    };
    // api log insert
    LogsController.APIlogger = function (ctx, options) {
        return __awaiter(this, void 0, void 0, function () {
            var guid, model, method, params, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        if (!!/^\/api\/log-(api|errors)$/.test(ctx.path)) return [3 /*break*/, 2];
                        guid = tools_1.Guid();
                        model = new api_1.API();
                        method = ctx.method;
                        model.id = guid;
                        model.ip = ctx.header['x-real-ip'] || ctx.req.connection.remoteAddress,
                            model.path = ctx.path;
                        model.url = ctx.url;
                        model.status = ctx.status;
                        model.origin = ctx.origin;
                        model.hostname = ctx.header['x-host'];
                        model.headers = ctx.header;
                        model.resHeaders = ctx.response.header;
                        model.resData = ctx.body;
                        model.protocol = ctx.protocol;
                        model.createdAt = Moment(Date.now()).format('YYYY/MM/DD HH:mm:ss.SSS');
                        model.createdBy = ctx.state['CUR_USER'] ? ctx.state['CUR_USER'].id : model.ip;
                        model.method = method;
                        if (method === 'GET') {
                            model.params = ctx.querystring;
                        }
                        else if (/^P(U|OS)T$/.test(method)) {
                            if (/^\/api\/login$/.test(ctx.path)) {
                                params = ctx.fields;
                                params['password'] = '******';
                            }
                            model.params = ctx.fields;
                        }
                        model.time = options.time; // deal time
                        return [4 /*yield*/, typeorm_1.getMongoManager('mongo').save(model)];
                    case 1:
                        result = _a.sent();
                        _a.label = 2;
                    case 2: return [2 /*return*/];
                }
            });
        });
    };
    // errors log insert
    LogsController.ERRlogger = function (ctx, options) {
        return __awaiter(this, void 0, void 0, function () {
            var guid, model, method, params, result;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        guid = tools_1.Guid();
                        model = new errors_1.Errors();
                        method = ctx.method;
                        model.id = guid;
                        model.ip = ctx.header['x-real-ip'] || ctx.req.connection.remoteAddress,
                            model.path = ctx.path;
                        model.url = ctx.url;
                        model.origin = ctx.origin;
                        model.hostname = ctx.header['x-host'];
                        model.headers = ctx.header;
                        model.resHeaders = ctx.response.header;
                        model.resData = ctx.body;
                        model.protocol = ctx.protocol;
                        model.createdAt = Moment(Date.now()).format('YYYY/MM/DD HH:mm:ss.SSS');
                        model.createdBy = ctx.state['CUR_USER'] ? ctx.state['CUR_USER'].id : model.ip;
                        model.status = options.status;
                        model.errors = options.errors;
                        model.msg = options.msg;
                        model.method = method;
                        if (method === 'GET') {
                            model.params = ctx.querystring;
                        }
                        else if (/^P(U|OS)T$/.test(method)) {
                            if (/^\/api\/login$/.test(ctx.path)) {
                                params = ctx.fields;
                                params['password'] = '******';
                            }
                            model.params = ctx.fields;
                        }
                        model.time = options.time; // deal time
                        return [4 /*yield*/, typeorm_1.getMongoRepository(errors_1.Errors, 'mongo').save(model)];
                    case 1:
                        result = _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    return LogsController;
}());
exports.default = LogsController;
//# sourceMappingURL=LogsController.js.map